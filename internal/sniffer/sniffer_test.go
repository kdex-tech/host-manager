...
func TestRequestSniffer_sniff_A(t *testing.T) {
	tests := []struct {
		...
		{
			// Regression for kdex-tech/host-manager#31. The proposed function
			// path /v1/admin/invites is already declared on a DIFFERENT
			// existing function (user-service-admin). Sniff must return nil
			// — creating a second function for the same path produces a
			// "duplicated path" error in KDexInternalHost reconciler that
			// aborts every mux rebuild on that host.
			name: "GET /v1/admin/invites skipped when another function already declares that path",
			r:    httptest.NewRequest("GET", "/v1/admin/invites", http.NoBody),
			functions: []kdexv1alpha1.KDexFunction{
				{
					ObjectMeta: metav1.ObjectMeta{
