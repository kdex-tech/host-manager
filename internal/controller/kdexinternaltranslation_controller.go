...
func (r *KDexInternalTranslationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if translation.Spec.HostRef.Name != r.FocalHost {
		log.V(4).Info("skipping reconcile", "host", translation.Spec.HostRef.Name, "focalHost", r.FocalHost)
		return ctrl.Result{}, nil
	}

	// Snapshot the observed status so the deferred write can be made
	// conditional on an actual change.
	observedStatus := translation.Status.DeepCopy()
	...
	defer func() {
		...
		if !objectStatusEqual(observedStatus, &translation.Status) {
