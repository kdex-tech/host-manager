...
func (r *KDexInternalHostReconciler) collectInitialPaths(
	...
	for _, function := range functions.Items {
		// internal functions are reachable only at their cluster-local
		// Knative URL; keep them out of the host's registered paths so they
		// are neither advertised in /-/openapi nor routable through the host
		// mux. See kdex-tech/kdex-crds#6.
		if function.Spec.Internal {
			continue
		}
		pathInfo := ko.PathInfo{
			API:           *ko.FromKDexAPI(&function.Spec.API),
			AutoGenerated: function.Spec.GetMetadata().AutoGenerated,
			Metadata:      function.Spec.GetMetadata().Metadata,
...
func (r *KDexInternalHostReconciler) createOrUpdateBackendDeployment(
	...
	_, err := ctrl.CreateOrUpdate(
		...
		func() error {
			...
			// Size() is nil-safe on the generated pointer receiver, so a > 0
			// result already implies Resources is set.
